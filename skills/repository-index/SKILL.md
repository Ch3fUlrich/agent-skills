...
| [`unattended-orchestration`](../unattended-orchestration/SKILL.md) | Internal (from `basic-analysis`) | — | Headless background sessions in isolated worktrees, parallel lanes, usage-limit recovery, guard-gated auto-merge | Work runs for **hours or overnight with nobody watching**. The dividing line from `herdr-orchestration` is *who recovers a stopped session*: there, a human looking at a pane; here, nobody until morning. |
