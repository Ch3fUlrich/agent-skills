...
| **`context7`** | Up-to-date library/framework docs | Any question about a library, framework, SDK, or CLI — *even one you think you know*. Training data goes stale; this does not. | User-scope, **global npm install driven by `node`** — never `npx -y`, which re-resolves the registry every spawn (3704 ms vs 1316 ms, and >30 s under concurrency: it times out). API key sits in plain `args`. |
| **`playwright`** | Browser automation | Strictly UI validation, screenshot testing, E2E against sites you build. | One user-scope entry, serving every repo. Transport per host: **global npm install + `node`** where node exists (392 ms vs 3521 ms for `npx @latest`), Docker (`mcr.microsoft.com/playwright/mcp`, `--network host`) on `coding.vm`, which has no node. **Tool prefix follows the wiring** — user-scope is `mcp__playwright__*`, a plugin is `mcp__plugin_playwright_playwright__*`. See `skills/mcp-servers-setup/SKILL.md` → Playwright. |
...
| [`unattended-orchestration`](../unattended-orchestration/SKILL.md) | Internal (from `basic-analysis`) | — | Agent sessions in isolated worktrees, parallel lanes, usage-limit recovery, guard-gated auto-merge. **Portable**: `-Init` in any git repo; the agent is a swappable adapter | Work runs for **hours or overnight with nobody watching**, or you are **adopting** unattended orchestration into a repository. The dividing line from `herdr-orchestration` is *who recovers a stopped session*: there, a human looking at a pane; here, nobody until morning. |
