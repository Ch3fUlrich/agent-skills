...
class LocalGLMAdapter:
    ...
    def invoke(self, request: AgentRequest) -> AgentResponse:
        ...
        api_key = KeyManager.get("local_glm", "GLM_API_KEY", "GLM_API_KEY")
        if not api_key:
            return AgentResponse(
                agent_id=request.agent_id,
                role=request.role,
                status="failed",
                content="Missing GLM_API_KEY from agent_keys.yaml or environment for local_glm provider.",
            )

        base_url = KeyManager.get("local_glm", "GLM_BASE_URL", "GLM_BASE_URL") or "https://open.bigmodel.cn/api/paas/v4"
