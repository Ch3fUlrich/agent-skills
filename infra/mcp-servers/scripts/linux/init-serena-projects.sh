...
for repo in "$CODE_ROOT"/*/; do
    ...
    if [ -f "$repo/.serena/project.yml" ]; then
        ...
        printf 'n\n%.0s' {1..5} | serena project create "$repo" --language python --language html --language javascript --language typescript --language css --language scss --language markdown --language yaml --language csharp --language bash --language powershell --language json --language toml --language rust --language cpp --language go --index 2>/dev/null && COUNT=$((COUNT + 1)) && echo -e "  \033[32mv Created + indexed\033[0m" || { ERRORS=$((ERRORS + 1)); echo -e "  \033[31mX Error\033[0m"; }
    fi
done
