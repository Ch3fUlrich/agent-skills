...
import { buildSkillIntelligenceIndex, composeWorkflow, recommendSkills, semanticSearchSkills, validateNextSkill, validateWorkflow, } from "./intelligence.js";
export function registerTools(server, skills, skillsDir) {
    const skillMap = new Map();
    for (const skill of skills) {
        skillMap.set(skill.directoryName, skill);
    }
    const availableSkillNames = new Set(skills.map((skill) => skill.directoryName));
    const sessionSkillHistory = [];
    let intelligenceIndexPromise = null;
    ...
    server.tool("list_skills", "List all available superpowers skills with their descriptions and supporting files", async () => {
    ...
    server.tool("use_skill", "Load a superpowers skill by name. Returns the full skill content to follow as instructions. Optional guardrails can enforce required workflow order for the current goal.", {
        ...
        if (enforce_guardrails) {
            const guardrail = validateNextSkill(goal ?? "", sessionSkillHistory, name, availableSkillNames);
            if (!guardrail.valid) {
                return {
                    content: [
                        {
    ...
    server.tool("get_skill_file", "Load a supporting file from a superpowers skill", {
    ...
    server.tool("recommend_skills", "Recommend the most relevant skills for a task using semantic ranking and workflow policy boosts.", {
        ...
        const recommendations = recommendSkills(index, task, repo_context, max_results ?? 5);
        return {
            content: [
                {
                    type: "text",
    ...
    server.tool("compose_workflow", "Compose an ordered multi-skill workflow for a goal using guardrails and semantic relevance.", {
    ...
    server.tool("validate_workflow", "Validate whether selected skills satisfy required guardrails for a goal.", {
    ...
    server.tool("semantic_search_skills", "Semantic search across all skill markdown and supporting files.", {
