...
export function registerResources(server, skills, skillsDir) {
    ...
    for (const skill of skills) {
        ...
        server.resource(`${skill.directoryName}-skill`, uri, {
        ...
        for (const file of skill.files) {
            ...
            const mimeType = file.name.endsWith(".ts")
                ...
                : file.name.endsWith(".sh")
                    ? "text/x-shellscript"
                    : "text/markdown";
            server.resource(`${skill.directoryName}-${file.name}`, fileUri, {
                ...
                const content = await readFile(join(skillsDir, file.relativePath), "utf-8");
                return {
    ...
    server.resource("skill-file", new ResourceTemplate("superpowers://skills/{skillName}/{fileName}", {
        list: async () => {
            const resources = [];
            for (const skill of skills) {
                resources.push({
        ...
        const content = await readFile(join(skillsDir, fileEntry.relativePath), "utf-8");
        return {
